...
def initLPMatrices(params,Dri_Net):
    #Initializing the matrix for the LP
    A = np.zeros((params['NUM_DRI_NODES'], params['NUM_DRI_NODES']*(params['NUM_DEM_NODES']+params['NUM_PARALLEL_LINKS'])))
    for i in range(params['NUM_DRI_NODES']):
        for j in range(params['NUM_DRI_NODES']*(params['NUM_DEM_NODES']+params['NUM_PARALLEL_LINKS'])):
            if (j < (i+1)*(params['NUM_DEM_NODES']+params['NUM_PARALLEL_LINKS'])) and (j >= i*(params['NUM_DEM_NODES']+params['NUM_PARALLEL_LINKS'])):
                ...
                if (k<params['NUM_DEM_NODES']):
                    drivernode = Dri_Net.drivernodes[i]
                    demandnode = Dri_Net.demandnodes[k]
                    if (demandnode[2] == False and drivernode[0] == demandnode[0]) or (demandnode[2] == True and params['SubMatrix'][(drivernode[0], demandnode[0])] == True):
                        A[i,j] = 1
                else:   
                    A[i,j] = 1    


    G = np.zeros((params['NUM_DEM_NODES'] + 2*params['NUM_DRI_NODES']*params['NUM_PARALLEL_LINKS'] + params['NUM_DEM_NODES']*params['NUM_DRI_NODES'], params['NUM_DRI_NODES']*(params['NUM_DEM_NODES']+params['NUM_PARALLEL_LINKS'])))
